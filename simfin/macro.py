...
class macro:
    ...
    def grow_cons(self,eco,year):
        ...
        rate = 1.0+self.infl
        if igra == True:
            rate += 1/self.g_pars.loc['alpha_L',1]*self.gr_A  # Part TFP non lié à l'éducation (50%)
    ...
    def gdp(self):
        ...
        return
    def grow(self,year):
        self.gr_Y = 1/self.g_pars.loc['alpha_L',1]*self.gr_A + self.gr_L
        self.gr_Yp = 1/self.g_pars.loc['alpha_L',1]*self.gr_A + self.gr_Lp
        self.gdp()
        self.year = year
        return
...
class covid:
    ...
    def predict(self,outcome):
        ...
        if outcome=='cons':
            ...
            cons = pd.read_csv(module_dir+file_profiles+'cons.csv').to_frame()
            cons.columns = ['cons']
